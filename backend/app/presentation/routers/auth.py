...
from app.presentation.deps import auth_rate_limit, employee_repo
...
def auth_admin(data: AuthAdminRequest, employees=Depends(employee_repo),
               _rl: None = Depends(auth_rate_limit)):
    ident = auth_uc.authenticate_admin(data, MASTER_ADMIN_PASSWORD, employees)
